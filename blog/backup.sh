...
# 备份数据库
sql_target="/tmp/typecho.sql"
test -f "$sql_target" && rm "$sql_target"
sudo docker exec postgres pg_dump -U postgres typecho > $sql_target
md5sum "$sql_target" > "$sql_target.md5sum"

# 打包
echo "Try to pack..."
backup_file="$backup_dir/typecho.`date +%s`.tar.gz"
